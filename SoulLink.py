...
def addMons():
    ...
    if p1Entry.get() != '' and p2Entry.get() != '':
...
def showAll():
    ...
    if p1Team:
        for idx, mon in enumerate(p1Team, start=0):
            allMons += f'{p1Team[idx][1]}: {p1Team[idx][0]}\t<-->\t{p2Team[idx][0]}\n'
...
def findLink():
    if p1Entry.get() != '':
        ...
        try:
            if links[p1Entry.get()]:
                p2Entry.insert(0, links[p1Entry.get()])
    ...
    #If p2 nickname entered
    elif p1Entry.get() == '' and p2Entry.get() != '':
        p1Entry.delete(0, 'end')
        routeEntry.delete(0, 'end')
        ...
        try:
            if inv_links[p2Entry.get()]:
                p1Entry.insert(0, inv_links[p2Entry.get()])
        except KeyError:
